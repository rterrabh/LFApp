...
import com.ufla.lfapp.views.custom.SelectedRowTextView;
...
public class AutomatonMinimizationTable {
    ...
    private static final String[] PARAMETERS =
            ResourcesContext.getString(R.string.min_fsa_table_header).split("#");
    ...
    public boolean isDistinctFinalAndNotFinal(int row) {
        ...
        if (finiteStateAutomaton.isFinalState(pairState.first) && !finiteStateAutomaton.isFinalState(pairState.second)) {
            setDistinct(row);
            setReason(row, pairState.first.getName() + " " + Symbols.BELONGS_TO + " F, "
                + pairState.second.getName() + " " + Symbols.NOT_BELONGS_TO + " F");
            return true;
        } else if (!finiteStateAutomaton.isFinalState(pairState.first) &&
                finiteStateAutomaton.isFinalState(pairState.second)) {
            setDistinct(row);
    ...
    public AutomatonMinimizationTable(Activity activity, FiniteStateAutomaton finiteStateAutomaton) {
        this.activity = activity;
        this.finiteStateAutomaton = finiteStateAutomaton;
        init();
    }
    ...
    private void init() {
        ...
        TextView titleTableIndice = getTitleTableTextView(PARAMETERS[0]);
        Paint paint = titleTableIndice.getPaint();
        row.addView(titleTableIndice);
        row.addView(getTitleTableTextView(PARAMETERS[1]));
        row.addView(getTitleTableTextView(PARAMETERS[2]));
        row.addView(getTitleTableTextView(PARAMETERS[3]));
        tableLayout.addView(row);
        SortedSet<State> statesSet = finiteStateAutomaton.getStates();
