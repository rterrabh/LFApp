...
import com.ufla.lfapp.activities.automata.graph.views.SpaceWithBorder;
import com.ufla.lfapp.activities.automata.graph.views.VertexView;
...
//        betterVerticalVisualisation();
        ...
        float size = (Math.min(width / (float) actualNumColumns,
                height / (float) actualNumRows));
...
//        }
    ...
    public void drawAutomaton(AutomatonGUI automatonGUI) {
        clear();
        for (Map.Entry<State, Point> entry : automatonGUI.getStateGridPositions().entrySet()) {
    ...
    public AutomatonGUI getAutomatonGUI() {
        ...
        State initialState = (this.initialState == null) ? null : new State(this.initialState.getLabel());
...
//                    }
        ...
        return new AutomatonGUI(new Automaton(states, initialState, finalStates,
    ...
    private GridLayout.LayoutParams recoveryGridLayoutParams(EdgeView edgeView) {
        ...
        switch(edgeView.getEdgeDrawType()) {
            ...
            case REFLEXIVE_UP_EDGE_DRAW:
                ...
                break;
            default:
                throw new RuntimeException("EdgeDraw not definied!");
        }
        ...
        public boolean onSingleTapConfirmed(MotionEvent e) {
            ...
            if (isOnSelectErrorState()) {
                ...
                for (TransitionFunction transitionFunction : transitionFunctionsToCompAuto) {
                    addEdgeView(vertexViewByLabel.get(transitionFunction.getCurrentState()
                            .getName()),
                            vertexViewByLabel.get(transitionFunction.getFutureState().getName()),
                            transitionFunction.getSymbol());
                }
                transitionFunctionsToCompAuto = null;
                errorStateLabel = null;
